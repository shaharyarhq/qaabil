            ...
            <div class="fade-up-2 flex items-start justify-between gap-3 sm:gap-4 flex-wrap">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="flex items-center flex-wrap gap-2 sm:gap-3 text-sm text-[#94a3b8]">
                        ...
                        @if ($video->language)
                            <span class="text-[#e2e8f0]">·</span>
                            <span
                                class="inline-flex items-center gap-1 text-xs font-semibold text-[#1b3a6b] bg-[#eff6ff] border border-[rgba(27,58,107,.12)] rounded-md px-2 py-0.5">
                                🌐 {{ $video->language }}
                            </span>
                        @endif
                    </div>
                ...
                <div class="shrink-0 mt-0.5 sm:mt-1">
                    ...
                    @elseif ($status === 'pending')
                        <span
                            class="inline-flex items-center gap-1.5 text-[.7rem] font-bold uppercase tracking-[.06em] px-3 py-[.3rem] rounded-full text-[#94a3b8]"
                    ...
                    @else
                        <span
                            class="inline-flex items-center gap-1.5 text-[.7rem] font-bold uppercase tracking-[.06em] px-3 py-[.3rem] rounded-full text-red-500"
                        ...
                        <div class="flex flex-col gap-3">
                            @foreach ($video->learning_materials as $material)
                                ...
                                <a href="{{ asset('storage/' . $url) }}" target="_blank" rel="noopener"
                                    ...
                                    <svg class="w-4 h-4 text-[#94a3b8] shrink-0" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            @if ($video->quiz_link)
                                <a href="{{ $video->quiz_link }}" target="_blank" rel="noopener"
                                    ...
                                    <svg class="w-4 h-4 text-white/40 shrink-0" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                    </svg>
                            ...
                            @if (!empty($video->quiz_attachments))
                                <div class="flex flex-col gap-3">
                                    @foreach ($video->quiz_attachments as $attachment)
                                        @php
                                            $aName = is_array($attachment)
                                                ? $attachment['name'] ?? basename($attachment['url'] ?? 'Attachment')
                                                : $attachment;
                                        ...
                                        <a href="{{ asset('storage/' . $aUrl) }}" target="_blank" rel="noopener"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-semibold text-[#0f172a] truncate">
                                                    {{ $aName }}</p>
                                                <p class="text-xs text-[#94a3b8] mt-0.5">Quiz attachment · click to
                                                    open</p>
                                            </div>
                                            <svg class="w-4 h-4 text-[#94a3b8] shrink-0" fill="none"
                ...
                @else
                    ...
                    <div class="flex xl:flex-col gap-3 xl:gap-1 overflow-x-auto xl:overflow-x-visible pb-2 xl:pb-0 -mx-4 sm:-mx-6 xl:mx-0 px-4 sm:px-6 xl:px-0 scrollbar-hide">
                        @foreach ($related as $rel)
                            @php
                                $relStatus =
                                    $rel->status instanceof \App\Enums\VideoStatus ? $rel->status->value : $rel->status;
                                preg_match(
                                ...
                                $relThumbnail = $relVideoId
                                    ? "https://img.youtube.com/vi/{$relVideoId}/mqdefault.jpg"
                                    : null;
                            @endphp
                                ...
                                <div class="flex-1 min-w-0 py-0 xl:py-0.5">
                                    ...
                                    @php
                                        ...
                                        $relAvg =
                                            $relTotal > 0
                                                ? round(
                                                    $relReviews
                                                        ->map(fn($r) => optional($r->ratings->first())->value ?? 0)
                                                        ->avg(),
                                                    1,
                                                )
                                                : null;
                                    ...
                                    @if ($relAvg !== null)
                                        <div class="flex items-center gap-0.5 mb-1.5">
                                            ...
                                            @endfor
                                            <span
                                                class="text-[.7rem] font-bold text-[#f59e0b] ml-0.5">{{ number_format($relAvg, 1) }}</span>
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @else
                                            <span
                                                class="text-[.6rem] font-bold uppercase tracking-[.04em] px-2 py-[.2rem] rounded-full text-[#94a3b8]"
                                                style="background:rgba(27,58,107,.06);border:1px solid rgba(27,58,107,.1)">{{ $relStatus }}</span>
                                        @endif
                                        <span
                                            class="text-[11px] text-[#94a3b8]">{{ $rel->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </a>
                        @endforeach
