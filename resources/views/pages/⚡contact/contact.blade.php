<div>
    @php
        $contactSettings = app(\App\Settings\ContactPageSettings::class);

        $hero = $contactSettings->hero;
        $methods = $contactSettings->methods['items'];
        $socials = $contactSettings->socials['items'];
        $note = $contactSettings->note;
        $topics = $contactSettings->topics['items'];

        // Fixed brand colors per platform (not editable in admin)
        $socialColors = [
            'linkedin' => '#0077b5',
            'instagram' => '#e1306c',
            'facebook' => '#1877f2',
    ...
    <div class="hero bg-[#1b3a6b] relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-3xl mx-auto px-6 py-20 text-center">
            <div class="inline-flex items-center gap-2 rounded-full px-4 py-1.5 mb-6"
                ...
                <span
                    class="text-white/80 text-[.75rem] font-bold uppercase tracking-[.08em]">{!! str($hero['badge'])->sanitizeHtml() !!}</span>
            </div>
            ...
            <p class="text-white/60 text-lg max-w-lg mx-auto leading-relaxed">
                {!! str($hero['subtitle'])->sanitizeHtml() !!}
            </p>
        </div>
    </div>
            ...
            <div class="lg:col-span-2 flex flex-col gap-5">
                ...
                @foreach ($methods as $i => $m)
                    <a href="{{ $m['href'] }}"
                        ...
                        <div class="contact-icon-wrap w-11 h-11 rounded-xl flex items-center justify-center shrink-0 duration-200"
                            ...
                            @if ($m['type'] === 'whatsapp')
                                <img src="https://cdn.simpleicons.org/whatsapp/1b3a6b" width="18" height="18"
                                    alt="{{ $m['label'] }}" />
                            @else
                ...
                <div class="bg-white border border-[#e2e8f0] rounded-2xl p-5">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach ($socials as $s)
                            @php $color = $socialColors[$s['icon_key']] ?? '#1b3a6b'; @endphp
                            <a href="{{ $s['href'] }}" target="_blank" title="{{ str($s['icon_key'])->headline() }}"
                                ...
                                <img src="{{ $socialIconLinks[$s['icon_key']] }}" width="20" height="20"
                                    alt="{{ str($s['icon_key'])->headline() }}" />
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="fu d4 bg-[#1b3a6b] rounded-2xl p-6 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="text-[#f59e0b] text-2xl mb-3">{!! str($note['icon'])->sanitizeHtml() !!}</div>
                        <p class="text-sm font-bold text-white mb-1">{!! str($note['title'])->sanitizeHtml() !!}</p>
                        <p class="text-xs text-white/50 leading-relaxed">
                            {!! str($note['description'])->sanitizeHtml() !!}
                        </p>
                    </div>
                </div>
                ...
                <form wire:submit="send" class="flex flex-col gap-5">
                            ...
                            <select wire:model="country_code" id="country_code"
                                ...
                                @foreach (\App\Enums\CountryCode::cases() as $code)
                                    <option value="{{ $code->value }}"
                                        {{ $code === \App\Enums\CountryCode::default() ? 'selected' : '' }}>
                                        {{ $code->label() }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($topics as $t)
                                <button type="button" wire:click="$set('topic', '{{ $t }}')" <button
                                    {{ $attributes->class([
                                        'text-[.78rem] font-semibold px-3.5 py-1.5 rounded-full border transition-all',
                                        'bg-[#1b3a6b] text-white border-[#1b3a6b]' => $topic === $t,
                                        'border-[#e2e8f0] text-[#475569] bg-white hover:border-[#1b3a6b] hover:text-[#1b3a6b]' => $topic !== $t,
                                    ]) }}>
                                    {{ $t }}
                                </button>
                            @endforeach
