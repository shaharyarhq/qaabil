...
<nav class="bg-white border-b border-[#e2e8f0] sticky top-0 z-50" x-data="{ open: false }">
    <div class="max-w-8xl mx-auto px-8 py-2 flex items-center justify-between gap-4">
        ...
        <a {{ spa() }} href="/" class="shrink-0">
            <img src="{{ asset('/images/logo/qaabil.jpeg') }}" alt="Qaabil" class="h-14 w-auto object-contain">
        </a>
        ...
        <div class="flex items-center gap-2.5">
            ...
            @else
                <a href="{{ filament()->getPanel('member')->getLoginUrl() }}?redirectTo={{ url()->current() }}"
                    class="hidden sm:inline-block text-[.925rem] font-semibold text-[#475569] border-[1.5px] border-[#e2e8f0] rounded-[10px] px-[1.4rem] py-[.65rem] bg-transparent cursor-pointer transition-all hover:border-[#1b3a6b] hover:text-[#1b3a6b]">
                    Log in
                </a>
                <a href="{{ filament()->getPanel('member')->getRegistrationUrl() }}?redirectTo={{ url()->current() }}"
                    class="hidden sm:inline-block text-[.925rem] font-bold text-white bg-[#1b3a6b] border-[1.5px] border-[#1b3a6b] rounded-[10px] px-[1.5rem] py-[.65rem] cursor-pointer transition-colors hover:bg-[#122952]">
                    Sign up free
                </a>
            @endauth
{{-- 
            ...
            <button @click="open = !open"
                class="md:hidden flex flex-col justify-center items-center w-11 h-11 rounded-lg border border-[#e2e8f0] bg-white transition-colors hover:border-[#1b3a6b] hover:bg-[#eff6ff]"
                aria-label="Toggle menu">
                <span class="block w-6 h-[1.5px] bg-[#475569] transition-all duration-200 rounded"
                    :class="open ? 'rotate-45 translate-y-[5px]' : ''"></span>
                <span class="block w-6 h-[1.5px] bg-[#475569] transition-all duration-200 rounded my-[3.5px]"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-150"
        ...
        <div class="flex flex-col gap-1">
            <a {{ spa() }} href="{{ route('home') }}" @click="open = false"
                @class([
            ...
            <a {{ spa() }} href="{{ route('courses.index') }}" @click="open = false"
                @class([
                    'text-[1rem] font-semibold px-4 py-3.5 rounded-lg no-underline transition-colors',
                    'bg-[#eff6ff] text-[#1b3a6b]' => request()->routeIs('courses.*'),
                    'text-[#475569] hover:bg-[#eff6ff] hover:text-[#1b3a6b]' => !request()->routeIs(
            ...
            <a {{ spa() }} href="{{ route('pricing') }}" @click="open = false"
                @class([
                    'text-[1rem] font-semibold px-4 py-3.5 rounded-lg no-underline transition-colors',
                    'bg-[#eff6ff] text-[#1b3a6b]' => request()->routeIs('pricing'),
                    'text-[#475569] hover:bg-[#eff6ff] hover:text-[#1b3a6b]' => !request()->routeIs(
        ...
        <div class="mt-3 pt-3 border-t border-[#e2e8f0] flex flex-col gap-2">
            @auth
                ...
                @endphp
                <a href="{{ $homeUrl }}"
                    class="text-[1rem] font-semibold text-center text-[#475569] border-[1.5px] border-[#e2e8f0] rounded-[10px] px-4 py-3.5 transition-all hover:border-[#1b3a6b] hover:text-[#1b3a6b]">
                    Dashboard
                </a>
            @else
                <a href="{{ filament()->getPanel('member')->getLoginUrl() }}?redirectTo={{ url()->current() }}"
                ...
                <a href="{{ filament()->getPanel('member')->getRegistrationUrl() }}?redirectTo={{ url()->current() }}"
                    class="text-[1rem] font-bold text-center text-white bg-[#1b3a6b] border-[1.5px] border-[#1b3a6b] rounded-[10px] px-4 py-3.5 transition-colors hover:bg-[#122952]">
                    Sign up free
                </a>
            @endauth
        </div>
